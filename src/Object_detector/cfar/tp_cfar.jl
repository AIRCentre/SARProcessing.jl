...
"""
function two_parameter_constant_false_alarm_rate(image::Matrix{T},background_size::Integer = 41,guard_size::Integer=21,pfa::Real=0.1) where T <: Real
    ...
    for i_row in 1:(padded_image_rows-background_size)
        for j_column in 1:(padded_image_colums-background_size)
            ...
            targets[i_row,j_column] = _two_parameter_constant_false_alarm_rate_pixel(window_under_test, guard_size,  pfa)
        end
    end
    return targets
end
...
"""
function _two_parameter_constant_false_alarm_rate_pixel(image::Matrix{T}, guard_size::Integer = 80, pfa::Real = 0.0001) where T <: Real

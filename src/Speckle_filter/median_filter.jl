@doc """
    median filter
"""
function speckle_median_filter(image::Matrix{T} where T<: Real ,filter_size::Vector{N} where N<:Integer =[3,3])::Matrix{Real}
    ...
    for i in 1:im_rows-filter_size[1]+1
        for j in 1:im_columns-filter_size[2]+1
            despeckled_image[i,j] = median(image[i:i+filter_size[1]-1,j:j+filter_size[2]-1])
        end
    end

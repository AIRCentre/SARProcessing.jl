...
function filter_mean_test()
    ...
    mean = SARProcessing.meanFilter([7,7]);
    check &= isapprox(sum(mean),1; atol = 0.1);
    check &= size(mean)==(7,7);
    #checking the mean filter applied on the image.
    mean_no_padding = SARProcessing.conv2d(image_with_objects, SARProcessing.meanFilter([3,3]));
    mean_with_padding = SARProcessing.conv2d(image_with_objects, SARProcessing.meanFilter([3,3]),1,"same");
